...
python train.py --model_name_or_path ./model/dense_global_model/7 \
--max_query_length 24 --max_doc_length 128 \
--data_dir ./data/${dataset}/preprocess \
--learning_rate 1e-4 --optimizer_str adamw \
--per_device_train_batch_size 128 \
...
--num_train_epochs 30  \
--use_pq True \
--init_index_path ./data/${data_type}/evaluate/dense_global_model_7/OPQ96,PQ96x8.index \
--partition 96 --centroids 256 --quantloss_weight 0.0 \
|tee ./log/${savename}.log
```
where the `./model/dense_global_model/7` and `./data/${data_type}/evaluate/dense_global_model_7/OPQ96,PQ96x8.index` is generated by conventional workflow.
...
--Recall_cutoff 5 10 30 50 100 \
--ckpt_path ./model/${savename}/${epoch}/ \
--init_index_path ./data/${data_type}/evaluate/dense_global_model_7/OPQ64,PQ64x8.index \
--mode train \
...
python ./inference_and_test/post_verification.py \
--data_type ${data_type} \
--output_dir  evaluate/${savename}_${epoch} 
--candidate_from_ann 
--MRR_cutoff 10 \
